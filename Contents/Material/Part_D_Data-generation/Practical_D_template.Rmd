...
9. **Verify if object `draw1` is equal to `draw3` and if object `draw2` is equal to `draw4`. (Tip: use `all.equal()`)**
```{r}
...
12. **Draw two variables of size `1000` from a multivariate normal distribution, both with mean `5` and variance `1` and make sure that they correlate $\rho = .8$. Use the function `rmvnorm` from the package `mvtnorm`.**
