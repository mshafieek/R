...
1. a function name
2. a set of arguments surrounded by parentheses `( )`
3. a body of code surrounded by curly braces `{ }`
4. default values
5. what the function will return
...
Or make use of the pop-up help:
...
## Matrices
`matrix()` creates arrays with specified dimensions
```{r}
rvect <- matrix(data=vector, nrow=6, ncol=1)
rvect
cvect <- matrix(data=vector, nrow=1, ncol=6)
cvect
```
...
Lists are flexible data structures: the elements in a list may be a combinatiaon of different data types (numeric, character) and dimensions.
...
Square brackets `[]` are used to call single elements
