...
- The R Markdown cheat sheet, see menu Help -> Cheat Sheets -> R Markdown Cheat Sheet or R Markdown Reference Guide.
...
1. Read the following short lesson about code chunks on this [`R Studio webpage`](https://rmarkdown.rstudio.com/lesson-3.html). 
...
**Insert a new code chunk and create `b` and assign `a+a`. Check if `b` is indeed `a+a`. **
```{r}
b <- a + a
b == a + a
```
...
**Round the answer from the previous exercise to 3 decimals in a new code chunk.**
```{r}
...
## Exercise 4: Inspect the contents of the global environment by typing and running `ls()`
...
There is are 4 objects in the environment: `a`, `b`, `c`, and `d`. If you look at the environment pane, you can quickly see the same information (i.e. there is no need to type `ls()`) and more, because you also see that these objects contain values. 
